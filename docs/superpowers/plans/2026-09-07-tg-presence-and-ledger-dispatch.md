...
**Architecture:** `mesh-promises` remains the authoritative materialized obligation view. A staffing query derives live windows and their current work state from existing tmux, charter, and Ledger artifacts, then excludes communication and protected substrate roles before proposing a real owner. `mesh-dispatch` emits only explicit owner-tagged tasks, and every assignment, decline, and retry remains visible as a Ledger promise.
...
- All delegation must use `mesh-task` and board events so `mesh-promises --feed` and `mesh-promises --check` can account for it.
...
- Exclude `tg`, `tg-roz`, human-owned work, substrate single-writer work, and windows with stale or blocked ownership unless an explicit operator-directed override exists.
...
- Produces: a versioned policy with `window`, `role`, `live`, `protected`, `open_promises`, `open_holds`, `eligible`, `reason`, and `observed_at`.
...
- [ ] Document exact exclusion and eligibility predicates, including fail-closed behavior when census or Ledger inputs cannot be read.
- [ ] Add a mutation that marks `tg` free and confirm the fixture still rejects `tg` as a worker.
...
- [ ] Run the focused test and verify missing command and fields fail.
- [ ] Implement the query with bounded subprocess calls, explicit timestamps, and structured reasons for every rejected window.
...
- `mesh-task create` preserves an explicit plan owner; an unowned task receives the selected owner in the emitted board tail before the promise feed.

- [ ] Add a failing fixture with an open task, free `haunt`, busy `genome`, and `tg` as poster; assert the task is offered only to `haunt`.
...
- [ ] Add a four-step plan fixture and assert `mesh-promises --feed` reports four obligations with only step one dispatchable.
- [ ] Add a mutation that deletes one plan row and assert the plan/ledger cardinality check fails.
...
- [ ] Run the read-only census against live tmux and Ledger state before enabling dispatch.
...
plan adds the missing operator-channel protection, live-window eligibility predicates, and staffing
pilot. It does not authorize automatic reassignment of substrate or human-owned work.
