...
- **Our hostnames / usernames** (`imozerov-*`, `Default-string`, `Redmi`, `imozerov@`, `<phone-user>` live; `<phone-user>` legacy) in:
...
- **bootstrap.sh**: parameterized (`PEER=${1:-${MESH_PEER:-<mind-ip>}}`) but the *default* is our IP.
