...
`mesh-wifi-motion` (WiFi passive radar: detect room motion from RSSI delta between consecutive scans over SSH to phone; STILL/MOTION/UNCERTAIN from ±2/5 dBm thresholds; `--edge` for cron wiring, `--test` = exit 2 if phone unreachable; feeds mesh-room-sense WiFi axis). Decayed
