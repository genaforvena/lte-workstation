...
`mesh-tv` (Samsung Smart TV organ — send a YouTube/video URL to the Samsung TV on the LAN: SSDP discovery → Tizen WebSocket API (samsungtvws) → opens URL in TV browser; `--discover` finds+saves TV IP, `--key KEY_HOME` sends IR commands; TV must be on + same LAN; pairing prompt on first connect; dep: samsungtvws; exit 2 = TV not found) ·
`mesh-url-watch` (LLM-free TV-organ reflex: watches voice-in.log for video URLs from operator Telegram → calls mesh-tv to play on Samsung TV; offset-based so only new lines trigger; wired */1 cron; exit 2 = n/a if voice-in.log or mesh-tv absent) ·
`mesh-sensor-log` (append-only time-series sensor log: records wifi_rssi, cpu_temp, room_sense (cached), phone_battery/light/accel to ~/.mesh/sensors.log per run; designed for pattern-discovery over time, no schema; */5 cron; phone data via mesh-phone-ip so ADB tunnel fallback applies). Decayed
