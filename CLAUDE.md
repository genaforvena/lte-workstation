...
- **Liveness / self-tend:** `mesh-card [--refresh]` · `mesh-health` · `mesh-hw-health` · `mesh-egress-health` · `mesh-supervise` · `mesh-verify` · `mesh-tick`/`mesh-heartbeat`/`mesh-beacon-watch`/`mesh-selfcare` · `mesh-router-watch` · `mesh-body-power` · `mesh-node-power` · `mesh-reflex-health` · `mesh-therm-watch` · `mesh-mind-state` · `mesh-resource-guard`.
