...
`mesh-sensor-log` (append-only time-series sensor log: wifi_rssi/cpu_temp/phone_battery/light/steps/room_sense to ~/.mesh/sensors.log; --tail/--grep/--diff/--edge subcommands; --diff emits MOVED/SAME_ROOM verdict; --edge = */5 cron reflex, posts [room-moved] to board once on dominant-AP change, seeds silently on first run; phone via mesh-phone-ip ADB tunnel) ·
