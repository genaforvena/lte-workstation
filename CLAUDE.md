...
`mesh-sms-send` (phone-side SMS helper: runs on the phone body; called by mesh-sms over SSH) ·
...
`mesh-mode` (quota-budget MODE switcher: dormant/focus/normal/sprint; edits ~/.mesh/reflexes.cron durably [healer-safe] + full crontab replace; dormant=plan/30min only, focus=plan+1worker, normal=3streams, sprint=3fast; on-demand, no cron) ·
