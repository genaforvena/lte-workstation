...
repo="$tmp/repo"
mkdir -p "$repo/scripts/reticulum" "$repo/scripts/tests" "$repo/scripts/__pycache__" "$repo/scripts/ux/chibicc" "$repo/job" "$repo/outside"
...
printf 'chibicc fixture\n' > "$repo/scripts/tests/chibicc.fixture"
ln -s ../../tests/chibicc.fixture "$repo/scripts/ux/chibicc/tests"
ln -s ux "$repo/scripts/uxn"
...
grep -q $'^scripts/ux/chibicc/tests\t\t.*\tfixture\tnone\tnone\t' "$live"
grep -q $'^scripts/uxn\t\tux\tlibrary\tnone\tnone\t' "$live"
