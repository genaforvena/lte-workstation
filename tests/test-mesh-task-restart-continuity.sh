...
base=(MESH_DIR="$td/mesh" MESH_MISHE_HOME="$td/mishe" MESH_BIN="$td/bin" MESH_TASK_DIR="$td/mesh/chains" MESH_TASK_CHAT_CMD="$td/bin/mesh-chat" MESH_TASK_HANDOFF_CMD="$td/bin/mesh-handoff" MESH_TASK_MAX_ACTIVE=1 TEST_BOARD="$td/board" TEST_REPO="$repo" PATH="$td/bin:$PATH")
