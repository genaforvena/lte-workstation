...
description: Use when adding, editing, or removing a rule or invariant in AGENTS.md — the shared base instruction every mind receives. Keeps rule IDs unique, the registry rev moving, and both gates green.
---

# Mesh invariants

`AGENTS.md` (repo root) is the one editable, engine-neutral list of procedural
...
1. Read `AGENTS.md` in full. Rules live under `## Mesh operating contract` as `mesh:N` bullets;
...
4. Verify, by your own hand, both gates:
   ...
   - `mesh-rules --check <path-to-edited-AGENTS.md>` → PASS
